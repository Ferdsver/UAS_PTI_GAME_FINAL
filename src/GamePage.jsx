...
function GamePage({ character, onQuit }) {
  ...
  const [gameTime, setGameTime] = useState(() => {
    const now = new Date("2025-06-12T18:25:00+07:00");
  ...
  useEffect(() => {
    /* Game Over Check */
    if (playerData && playerData.currentHealth <= 0 && !isGameOver) {
      setIsGameOver(true);
    }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      e.preventDefault(); // Cegah key repeat
      ...
      setKeysPressed((prevKeys) => {
        if (prevKeys[key]) return prevKeys; // Hindari pengulangan
  ...
  const buyItemFromWeaponShop = useCallback((item) => {
    setPlayerData((prev) => {
      ...
      blacksmithAudioRef.current
        .play()
        .catch((e) => console.error("Error playing blacksmith sound:", e));
  ...
  const buyItemFromFoodStall = useCallback((item) => {
    setPlayerData((prev) => {
      ...
      makanAudioRef.current
        .play()
        .catch((e) => console.error("Error playing makan sound:", e));
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (type === "battle") {
        ...
        healingAudioRef.current
          .play()
          .catch((e) => console.error("Error playing healing sound:", e));
        setInteractionMessage(message);
        setIsInteracting(true);
        healAtClinic();
      } else if (type === "rest") {
        snoringAudioRef.current.currentTime = 0;
        snoringAudioRef.current
          .play()
          .catch((e) => console.error("Error playing snoring sound:", e));
        setInteractionMessage(message);
        setIsInteracting(true);
        restAtRestArea();
      } else {
  ...
  // --- GAME LOOP UTAMA DENGAN FIXED TIMESTEP ---
  useEffect(() => {
    ...
    lastFrameTimeRef.current = performance.now();
    const FIXED_TIME_STEP = 1 / 60; // 60 updates per detik
    ...
    const updatePosition = (fixedDeltaTime) => {
      ...
      if (moveX !== 0 || moveY !== 0) {
        ...
        moveX = (moveX / magnitude) * currentEffectiveSpeedPPS * fixedDeltaTime;
        moveY = (moveY / magnitude) * currentEffectiveSpeedPPS * fixedDeltaTime;

        if (playerData.currentEnergy > 0) {
          setPlayerData((prev) => {
            if (!prev) return null;
            const energyDecayAmount =
              ENERGY_DECAY_ON_MOVE_PER_SECOND *
              fixedDeltaTime *
              speedMultiplier;
            const newEnergy = Math.max(
              0,
              prev.currentEnergy - energyDecayAmount
            );
        ...
        let newX = playerPositionRef.current.x + moveX;
        let newY = playerPositionRef.current.y + moveY;
    ...
    const gameLoop = (currentTime) => {
      ...
      while (accumulatedTime >= FIXED_TIME_STEP) {
        if (!isGameOver && !isInteracting) {
          updatePosition(FIXED_TIME_STEP);
  ...
  const handleJoystickInteraction = (direction, isPressed) => {
    if (isInteracting || isGameOver) return;
    setJoystickPressed((prev) => {
      if (prev[direction] === isPressed) return prev;
      return { ...prev, [direction]: isPressed };
    });
  };
